...
def render(spreadsheet, get_kr_price, gold_override):
    ...
    # ── 5행: 매입단가 vs 현재가 산점도 ────────────────────
    st.markdown("---")
    st.markdown("##### 매입단가 vs 현재가 (버블: 평가총액 크기)")

    # 종목별로 집계 (가중평균 매입단가)
    pivot_scatter = (
        df_valid.groupby("종목명", as_index=False)
        .apply(lambda g: pd.Series({
            "매수단가(가중평균)": (g["매입총액"].sum() / g["보유수량"].sum()),
            "현재가": g["현재가"].iloc[0],
            "평가총액": g["평가총액"].sum(),
            "수익률(%)": (g["평가총액"].sum() / g["매입총액"].sum() - 1) * 100,
        }))
        .reset_index(drop=True)
    )

    price_min = min(pivot_scatter["매수단가(가중평균)"].min(), pivot_scatter["현재가"].min()) * 0.9
    price_max = max(pivot_scatter["매수단가(가중평균)"].max(), pivot_scatter["현재가"].max()) * 1.1
    ...
    fig7 = px.scatter(
        ...
        color="수익률(%)",
        text="종목명",
        color_continuous_scale=["#ef553b", "#aaaaaa", "#00cc96"],
    ...
    fig7.add_shape(
        type="line",
        x0=price_min, y0=price_min,
        x1=price_max, y1=price_max,
        line=dict(color="gray", dash="dash", width=1),
    )
    fig7.update_traces(textposition="top center")
    fig7.update_layout(
        xaxis_title="매수단가 (가중평균, KRW)",
        yaxis_title="현재가 (KRW)",
        coloraxis_colorbar_title="수익률(%)",
        margin=dict(t=20),
    )
    st.plotly_chart(fig7, use_container_width=True)
    st.caption("점선: 손익분기선 (매입단가 = 현재가). 선 위 = 수익, 선 아래 = 손실. 버블 크기 = 평가총액.")
