...
if menu == "Table" and submenu == "해외 투자자산":
    ...
    # ✅ USD 합계
    total_buy_usd = df["매입총액(LC)"].sum()
    total_eval_usd = df["평가총액(LC)"].sum()
    total_yield_usd = (total_eval_usd / total_buy_usd - 1) * 100 if total_buy_usd else 0

    st.markdown(f"""
    <div style='display:flex;gap:60px;font-size:1.1em;font-weight:bold;flex-wrap:wrap;'>
        <div>
            해외 자산 매입총액<br>
            KRW: {fmt_num(total_buy)} 원<br>
            USD: ${fmt_num(total_buy_usd)}
        </div>
        <div>
            해외 자산 평가총액<br>
            KRW: {fmt_num(total_eval)} 원<br>
            USD: ${fmt_num(total_eval_usd)}
        </div>
        <div>
            해외 자산 전체 수익률<br>
            KRW 기준: {fmt_pct(total_yield)}<br>
            USD 기준: {fmt_pct(total_yield_usd)}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    display_df = df[show_cols].copy()
    money_cols = [c for c in display_df.columns if "총액" in c or "손익" in c or c in ["보유수량","매수단가","매입환율","현재가"]]
